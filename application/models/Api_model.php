...
class  Api_model extends CI_Model
{
    ...
    public function insertorder($orderno,$userid,$proid,$detail,$ischarge,$state,$mark,$ip,$time,$updatedbtime)
    {
        //$time = date("Y-m-d H:i:s");
        $query = "insert into api_order VALUE (0,?,?,?,?,?,?,?,?,?,?,?)";
        return $this->db->query($query,array("$orderno","$userid","$proid","$detail","$ischarge","$state","$mark","$ip","$updatedbtime","$time","$time"));
    }
